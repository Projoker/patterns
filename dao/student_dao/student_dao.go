...
func New(path string) *StudentDAO {
	return &StudentDAO{path: path}
}

// GetAllStudents - Список всех студентов
func (s *StudentDAO) GetAllStudents() ([]Student, error) {
...
func (s *StudentDAO) FindStudentByID(id string) (Student, error) {
	f, err := os.Open(s.path)
	defer f.Close()
...
func (s *StudentDAO) AddStudent(student Student) error {
